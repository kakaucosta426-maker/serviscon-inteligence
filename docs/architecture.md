...
O build executa `prisma generate && next build`. Seed e migrations não são executados automaticamente no deploy de produção; migrations devem ser aplicadas por `npm run db:deploy` em etapa controlada.
